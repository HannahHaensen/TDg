...
def render_depth(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0):
    """
    MODIFICATION 2 (Corrected): Render depth map using the 'pseudo expected depth' method.
    This involves two rendering passes performed robustly within this function.
    """
    device = bg_color.device
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        tanfovx=tanfovx,
        tanfovy=tanfovy,
        bg=torch.zeros(3, device=device),  # Use black background for alpha and depth passes
        scale_modifier=scaling_modifier,
        viewmatrix=viewpoint_camera.world_view_transform,
        projmatrix=viewpoint_camera.full_proj_transform,
        sh_degree=pc.active_sh_degree,
    ...
    # PASS 1: Render a field of ones to get the accumulated alpha.
    # The color of each gaussian is set to [1,1,1]. The resulting image's
    # channel values are equivalent to the accumulated alpha.
    colors_ones = torch.ones_like(means3D)
    alpha_pass_result, radii = rasterizer(
        means3D=means3D, means2D=torch.zeros_like(means3D), shs=None,
        colors_precomp=colors_ones, opacities=opacity, scales=scales, rotations=rotations)
    final_alpha = alpha_pass_result[0:1, ...]  # Take one channel as alpha

    # Calculate camera-space depth for each Gaussian center
    with torch.no_grad():
        points_homogeneous = torch.cat([means3D, torch.ones_like(means3D[:, :1])], dim=-1)
        points_camera = torch.matmul(points_homogeneous, viewpoint_camera.world_view_transform.T)
        ...
        if not torch.any(valid_mask):
            ...
            valid_depths = depths[valid_mask]
            depth_min, depth_max = torch.min(valid_depths), torch.max(valid_depths)

            if depth_max - depth_min > 1e-9:
                depths_normalized = (depths - depth_min) / (depth_max - depth_min)
            else:
                depths_normalized = torch.ones_like(depths) * 0.5

            depths_normalized[~valid_mask] = 0
            depths_normalized = torch.clamp(depths_normalized, 0.0, 1.0)

    # PASS 2: Render with normalized depth as the color.
    depth_colors = depths_normalized.unsqueeze(-1).repeat(1, 3)
    blended_depth_normalized, _ = rasterizer(
        means3D=means3D, means2D=torch.zeros_like(means3D), shs=None,
        colors_precomp=depth_colors, opacities=opacity, scales=scales, rotations=rotations)
    blended_depth_normalized = blended_depth_normalized[0:1, ...]

    # COMPOSITE: Calculate expected depth and un-normalize
    epsilon = 1e-6
    expected_depth_normalized = blended_depth_normalized / (final_alpha + epsilon)

    final_depth = expected_depth_normalized * (depth_max - depth_min) + depth_min

    # Use original background color for the final output visualization where alpha is low
    # But for the metric depth map, 0 is a better background value.
    final_depth[final_alpha < 0.1] = 0

    return {
        "render": final_depth,
        "render_vis": expected_depth_normalized,
        "viewspace_points": torch.zeros_like(pc.get_xyz),  # Dummy value
        "visibility_filter": radii > 0,
        "radii": radii,
    }
